...
export class CodeOptimizer {
    ...
    optimize(
        source: string,
        usedControllers: ControllerInfo[] = [],
        ...
        return {
            ...
            bundleInfo: {
                ...
                usedMethods: usedControllers.flatMap(c =>
     ...
     * Dead Code Elimination
     */
    private eliminateDeadCode(code: string): { code: string; removed: string[] } {
     ...
     */
    private shakeUnusedMethods(code: string, usedControllers: ControllerInfo[]): string {
        // 使用されているメソッド名を収集
     ...
     * 小さな関数をインライン化
     */
    private inlineSmallFunctions(code: string): { code: string; inlined: string[] } {
 ...
 * バンドルを分析
 */
export function analyzeBundle(files: Map<string, string>): BundleAnalysis {
 ...
 * 重複コードを検出
 */
function detectDuplicates(modules: ModuleInfo[]): DuplicateInfo[] {
 ...
 * 未使用エクスポートを検出
 */
function detectUnusedExports(modules: ModuleInfo[]): string[] {
 ...
 * 使用されているメソッドを収集
 */
export function collectUsedMethods(
    controllers: ControllerInfo[],
    rpcCalls: Array<{ controllerName: string; methodName: string }>
 ...
 */
export function filterUnusedMethods(
